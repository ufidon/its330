//ref: 
// 1. https://rosettacode.org/wiki/Dining_philosophers#Java
// 2. https://www.jitendrazaa.com/blog/java/compile-and-run-java-program-in-package-from-command-line/

// package diningphilosophers;
/** How to compile and run?
 * Compile: javac DinningPhilosophers.java
 * Run: java DinningPhilosophers
 */
...
class Philosopher implements Runnable {
    ...
    Philosopher() {
        id = instances++;
        left = DinningPhilosophers.forks.get(id);
        right = DinningPhilosophers.forks.get((id+1)%DinningPhilosophers.philosopherCount);
    }
        ...
        do {
            if (state == PhilosopherState.Pon) {    //  all that pondering
                ...
                if (token.get() == id) {            //  my turn now
                    ...
                    token.set((id+2)% DinningPhilosophers.philosopherCount);
