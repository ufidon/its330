...
int __cdecl main(void) 
{
    ...
    // Initialize Winsock
    printf("server: Initialize Winsock\n");
    ...
    printf("server: Create a SOCKET for listening clients.\n");
    ...
    printf("server: Bind the listening SOCKET.\n");
    ...
    do {
        ...
        if (iResult > 0) {
            printf("server: Bytes received: %d from the client.\n", iResult);
            printf("Server: The client said '%s' to me.\n", recvbuf);
            

        // Say hello  to the sender
            printf("server: I said '%s' to the client.\n", serverMsg);
    ...
    printf("server: Disconnect from the client.\n");
