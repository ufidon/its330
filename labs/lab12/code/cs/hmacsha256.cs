    ...
    public static void Main(string[] Fileargs)
    {
        ...
        if (Fileargs.Length < 1)
        {
            ...
            if (!File.Exists(dataFile))
            {
                ...
                using (StreamWriter sw = File.CreateText(dataFile))
                {
                    ...
                    Console.WriteLine("3. The content in the generated data file {0} is:\n\n{1}\n", dataFile, content);
                }
                Console.WriteLine("4. The hash file for the data file {0} is {1}\n", dataFile, hashFile);
            }
        }
        else
        {
        ...
        try
        {
            ...
            using (RNGCryptoServiceProvider rng = new RNGCryptoServiceProvider())
            {
                ...
                Console.WriteLine("5. The key for the HMAC-SHA256:");
                showinhex(secretkey, secretkey.Length);

                // Use the secret key to sign the message file.
                Console.WriteLine("6. Hashing file {0} with HMAC-SHA256...", dataFile);
                HashFile(secretkey, dataFile, hashFile);

                Console.WriteLine("7. The HMAC-SHA256 hash of {0} is saved in {1} together with its origninal content.\n\n", dataFile, hashFile);
                // Verify the signed file

                Console.WriteLine("8. Verifying the HMAC-SHA256 hash in {0} against {1}...", hashFile, dataFile);
        ...
        using (HMACSHA256 hmac = new HMACSHA256(key))
        {
            using (FileStream inStream = new FileStream(sourceFile, FileMode.Open))
            {
                using (FileStream outStream = new FileStream(destFile, FileMode.Create))
                {
                    ...
                    Console.WriteLine("6. The HMAC-SHA256 hash of file {0}:", sourceFile);
    ...
    public static bool VerifyFile(byte[] key, String sourceFile)
    {
        ...
        using (HMACSHA256 hmac = new HMACSHA256(key))
        {
            ...
            using (FileStream inStream = new FileStream(sourceFile, FileMode.Open))
            {
                ...
                Console.WriteLine("8. The stored HMAC-SHA256 hash is:");
                showinhex(storedHash, storedHash.Length);
                // Compute the hash of the remaining contents of the file.
                // The stream is properly positioned at the beginning of the content, 
                // immediately after the stored hash value.
                byte[] computedHash = hmac.ComputeHash(inStream);
                Console.WriteLine("8. The computed HMAC-SHA256 hash is:");
        ...
        if (err)
        {
            Console.WriteLine("8. Hash values differ! Hashed file has been tampered with!");
            return false;
        }
        else
        {
            Console.WriteLine("8. Hash values agree -- no tampering occurred.");
            return true;
        }
    } //end VerifyFile
} //end class
