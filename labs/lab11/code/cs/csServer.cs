  ...
  public static void Main(string[] args)
  {
      if (args.Length == 1)
      {
        ...
        try
            ...
            Console.Write("Server 01: Create TcpListener...\n");
            server = new TcpListener(localAddr, port);
            //server = new TcpListener(IPAddress.Any, port);

            // Start listening for client requests.
            Console.Write("Server 02: Start listening for client requests... \n");
            server.Start();
            ...
            while(true) 
            {
                Console.Write("Server 03: Waiting for a connection from clients or press ctrl+c to exit...\n");
                ...
                Console.Write("Server 04: accepted client from {0}. \n", remoteIpEndPoint.Address);        
                Console.WriteLine("Server 05: Connected to client {0}!", remoteIpEndPoint.Address);
                ...
                Console.Write("Server 06: keep reading data from client {0}. \n", remoteIpEndPoint.Address);
                while((i = stream.Read(bytes, 0, bytes.Length))!=0) 
                {   
                    // Translate data bytes to a ASCII string.
                    data = System.Text.Encoding.ASCII.GetString(bytes, 0, i);
                    Console.WriteLine("Server 06: Received message '{0}' from client {1} ", data, remoteIpEndPoint.Address);
                
                    // Process the data sent by the client.
                    data = data.ToUpper();
                    Console.WriteLine("Server 07: Convert client message to upcase");

                    byte[] msg = System.Text.Encoding.ASCII.GetBytes(data);

                    // Send back a response.
                    stream.Write(msg, 0, msg.Length);
                    Console.WriteLine("Server 08: Sent '{0}' back to client {1}", data, remoteIpEndPoint.Address);            
                }
                
                // Shutdown and end connection
                Console.WriteLine("Server 09: Disconnect from client {0}", remoteIpEndPoint.Address);
                client.Close();
            }
        }
        catch(SocketException e)
        {
            Console.WriteLine("Server 10: SocketException: {0}", e);
        }
        finally
        {
        ...
        Console.WriteLine("\nServer 11: Hit enter to continue...");
        Console.Read(); 
      }
      else
      {
